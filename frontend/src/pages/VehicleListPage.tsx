...
export default function VehicleListPage() {
  ...
  const { data, vehicles, isLoading, isLoadingMore, error, refetch } = useVehicles(filters);
      ...
      {!error && !isLoading && vehicles.length > 0 && (
        <motion.div layout className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          <AnimatePresence mode="popLayout">
            ...
            {/* Skeleton-Platzhalter fuer die im Hintergrund nachladenden
                Fahrzeuge (Position 7+ der aktuellen Seite). Zeigt sofort an,
                dass noch mehr kommt, ohne die bereits sichtbaren Karten zu
                blockieren oder zu verschieben. */}
            {isLoadingMore &&
              data &&
              Array.from({ length: Math.max(0, Math.min(data.pageSize, data.total) - vehicles.length) }).map(
                (_, i) => (
                  <motion.div key={`more-skeleton-${i}`} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <VehicleCardSkeleton />
                  </motion.div>
                )
              )}
